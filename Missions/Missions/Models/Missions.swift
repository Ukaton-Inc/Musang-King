...
class Missions {
    ...
    /// A helper method to calculate the slider value when wearing both smart shoe insoles
    /// - Parameters:
    ///   - leftValues: an array of left smart shoe insole sensor values whose values range between 0 and 4500
    ///   - rightValues: an array of right smart shoe insole sensor values whose values range between 0 and 4500
    func getSliderValue(leftValues: [Int], rightValues: [Int]) -> CGFloat {
        var leftSum: Int = 0
        var rightSum: Int = 0
        
        leftSum += leftValues.reduce(0, +)
        rightSum += rightValues.reduce(0, +)
        
        print("r - l: \(rightSum-leftSum)")
        
        let mapped = map(minRange: -16000, maxRange: 16000, minDomain: -1, maxDomain: 1, value: CGFloat(rightSum-leftSum))
        print("mapped: \(mapped)")
        print("left: \(leftSum)")
